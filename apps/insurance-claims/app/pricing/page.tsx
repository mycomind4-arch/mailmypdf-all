...
export const metadata = {
  title: 'Pricing — Insurance Claims',
  description: 'Transparent pricing for insurance claim and appeal workflows.',
}
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '3rem 1.5rem' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>{WORKFLOW_PRICING.map((w) => (
            <tr key={w.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <td style={tdStyle}><span style={bandBadge(w.band)}>{w.band}</span></td>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2.5rem' }}>
        {MAIL_PRICING.map((m) => (
          <div key={m.class} style={{ padding: '1.5rem', background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '0.625rem' }}>
            <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#67e8f9', marginBottom: '0.5rem' }}>{m.class}</p>
...
const sectionLabel: React.CSSProperties = { fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#67e8f9', marginBottom: '1rem', borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: '0.5rem' }
const thStyle: React.CSSProperties = { textAlign: 'left', padding: '0.625rem 0.75rem', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.7rem', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#64748b' }
const tdStyle: React.CSSProperties = { padding: '0.75rem', fontSize: '0.9rem', color: '#f8fafc' }
function bandBadge(band: string): React.CSSProperties {
  const colors: Record<string, string> = { STANDARD: '#67e8f9', ADVANCED: '#fb7185', ESSENTIAL: '#fbbf24' }
  return { display: 'inline-block', padding: '0.125rem 0.5rem', borderRadius: '0.25rem', fontSize: '0.7rem', fontWeight: 600, fontFamily: 'JetBrains Mono, monospace', color: colors[band] || '#64748b', background: 'rgba(15,23,42,0.8)' }
}
