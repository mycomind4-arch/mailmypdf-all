  ...
  switch (rule.type) {
    ...
    case "core-plus-optional":
      return evaluateRequirement(
        {
          type: "all",
          rules: rule.coreCapabilityIds.map((capabilityId) => ({ type: "capability", capabilityId })),
        },
        context,
      );
    case "threshold":
...
function hasAnyCapabilityProgress(group: WorkflowGroupDefinition, state: UserCapabilityState): boolean {
  return group.workflowIds.some(
    (workflowId) => state.inProgress.includes(workflowId) || state.completed.includes(workflowId),
  );
}
...
export function evaluateWorkflowGroup(
  ...
  const status: WorkflowGroupStatus = completion.satisfied
    ? "completed"
    : !unlock.satisfied
      ? "locked"
      : completedWorkflowCount > 0 || hasAnyCapabilityProgress(group, state)
        ? "in-progress"
        : "available";
