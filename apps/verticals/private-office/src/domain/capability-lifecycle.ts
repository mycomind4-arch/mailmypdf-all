/**
 * Converts authoritative workflow completion into deterministic capability,
 * milestone, and workflow-group state transitions.
 */
...
export interface CapabilityTransitionEvent {
  eventType: "capability_completed" | "milestone_reached" | "workflow_group_unlocked";
  matterId: string;
  ownerId: string;
  actorId: string | null;
...
export function applyCapabilityCompletion(
  ...
  if (!capability) throw new CapabilityTransitionError(`Unknown capability: ${input.capabilityId}`);
  ...
  const beforeAvailable = new Set(
    getAvailableWorkflowGroups(graph, groups, state, facts).map((evaluation) => evaluation.group.id),
  );
