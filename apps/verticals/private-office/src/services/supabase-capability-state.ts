...
async function insertEvent(
  ...
  const response = await fetch(eventsBase, {
    ...
    body: JSON.stringify({
      ...
      capability_id: typeof payload.capabilityId === "string" ? payload.capabilityId : null,
      milestone_id: typeof payload.milestoneId === "string" ? payload.milestoneId : null,
      workflow_group_id: typeof payload.groupId === "string" ? payload.groupId : null,
      matter_id: matterId ?? null,
      metadata: payload,
      created_at: new Date().toISOString(),
    }),
  });
  if (!response.ok) throw new Error(`Supabase capability event insert failed: ${response.status}`);
}
...
async function completeCapabilityForMatter(
  ...
  const result = applyCapabilityCompletion(
    ...
    currentState,
    { matterId, ownerId: userId, capabilityId },
  );
...
const repository = {
  async load(userId: string): Promise<UserCapabilityState> {
    ...
    if (!response.ok) throw new Error(`Supabase capability state load failed: ${response.status}`);
  ...
  async save(state: UserCapabilityState): Promise<void> {
    ...
    if (!response.ok) throw new Error(`Supabase capability state save failed: ${response.status}`);
  },
  ...
  async completeCapability(
    ...
    const result = applyCapabilityCompletion(
      ...
      businessWorkflowGroups,
      currentState,
      { matterId: `capability:${capabilityId}`, ownerId: userId, capabilityId },
    );
    ...
    return {
      ...
      newlyReachedMilestones: result.completion.newlyReachedMilestones.map((milestone) => ({
        id: milestone.id,
        title: milestone.title,
        description: milestone.description,
      })),
      newlyUnlockedCapabilities: result.completion.newlyUnlockedCapabilities.map((capability) => ({
        id: capability.id,
        title: capability.title,
      })),
      newlyUnlockedGroups: result.newlyUnlockedGroups.map((group) => ({
        id: group.id,
        title: group.title,
      })),
    };
  ...
  async startCapability(userId: string, capabilityId: string): Promise<UserCapabilityState> {
    const { eventsBase, key } = config();
    const currentState = await this.load(userId);
    const capability = capabilityGraph.capabilities[capabilityId];
    if (!capability) throw new CapabilityTransitionError(`Unknown capability: ${capabilityId}`);
    if (currentState.completed.includes(capabilityId) || currentState.inProgress.includes(capabilityId)) {
      return currentState;
    }
    ...
    await insertEvent(eventsBase, key, userId, "capability_started", { capabilityId });
    return newState;
  },
  ...
  async syncFromMatters(
    ...
    for (const workflowId of completedWorkflowIds) {
      const match = findCapabilityForWorkflow(capabilityGraph, workflowId);
      if (!match || state.completed.includes(match.capabilityId)) continue;
