...
import { cleanupRun } from "@mailmypdf/dev-agent-swarm";

const bodySchema = z.object({ runId: z.string().trim().min(1).max(200) });

export const Route = createFileRoute("/api/studio/agents/cleanup")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const result = await cleanupRun(repoRoot, parsed.data.runId);
