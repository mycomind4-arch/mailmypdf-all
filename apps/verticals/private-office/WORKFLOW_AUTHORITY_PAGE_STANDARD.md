# MailMyPDF Workflow Authority Page Standard

LOCKED cross-repo standard. Every workflow page is both an authority hub and an execution entry point.

Required sections:
1. Overview / search intent
2. When to use it
3. When not to use it
4. Official rules and authoritative sources
5. Deadlines and timing
6. Documents to gather
7. Information to confirm
8. Evidence checklist
9. How the workflow works
10. Issues and requirements checked
11. Common mistakes and failure modes
12. Representative scenarios
13. Practical checklists
14. Templates and tools
15. Workflow-specific FAQ
16. Glossary
17. Related workflows
18. Execution CTA
19. Mailing, tracking, and proof
20. Source freshness / last reviewed

High-stakes claims require current source verification. Never invent deadlines, eligibility, authorities, recipients, facts, or outcomes. Authority-page maturity is separate from workflow maturity: placeholder -> authority-draft -> source-verified -> workflow-wired -> gold -> production-verified.

Canonical public host: https://mailmypdf.ai. Prelaunch indexing remains disabled until the owner's launch switch is enabled.

Architecture source of truth: this monorepo's shared packages. Do not create a competing page structure, URL scheme, or global navigation model.
