...
-- used by Contractor Dispute and every future straight-line matter
-- workflow. Clients may read their own state, but only authenticated server
-- functions using the service role may mutate it — same shape as the
-- compound-matters tables above, sized for a step list instead of a phase
-- graph.
-- ─────────────────────────────────────────────────────────────────────────

create table if not exists public.private_office_step_matters (
  id uuid primary key,
  owner_id text not null,
  workflow_id text not null,
  version integer not null default 1 check (version > 0),
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint private_office_step_state_object_check check (jsonb_typeof(state) = 'object')
);

create index if not exists private_office_step_matters_owner_idx
  on public.private_office_step_matters(owner_id, updated_at desc);
create index if not exists private_office_step_matters_workflow_idx
  on public.private_office_step_matters(owner_id, workflow_id, updated_at desc);

create table if not exists public.private_office_step_events (
  id uuid primary key default gen_random_uuid(),
  matter_id uuid not null references public.private_office_step_matters(id) on delete cascade,
  owner_id text not null,
  event_type text not null check (
    event_type in (
      'step_matter_created',
  ...
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  constraint private_office_step_event_metadata_object_check
    check (jsonb_typeof(metadata) = 'object')
);

create index if not exists private_office_step_events_matter_idx
  on public.private_office_step_events(matter_id, created_at asc);
create index if not exists private_office_step_events_owner_idx
  on public.private_office_step_events(owner_id, created_at desc);

alter table public.private_office_step_matters enable row level security;
alter table public.private_office_step_events enable row level security;

drop policy if exists private_office_step_matters_select_own
  on public.private_office_step_matters;
create policy private_office_step_matters_select_own
  on public.private_office_step_matters
  for select
  to authenticated
  using ((select auth.uid())::text = owner_id);

drop policy if exists private_office_step_matters_insert_own
  on public.private_office_step_matters;
drop policy if exists private_office_step_matters_update_own
  on public.private_office_step_matters;
drop policy if exists private_office_step_matters_delete_own
  on public.private_office_step_matters;

drop policy if exists private_office_step_events_select_own
  on public.private_office_step_events;
create policy private_office_step_events_select_own
  on public.private_office_step_events
  for select
  to authenticated
  using ((select auth.uid())::text = owner_id);
drop policy if exists private_office_step_events_insert_own
  on public.private_office_step_events;
drop policy if exists private_office_step_events_update_own
  on public.private_office_step_events;
drop policy if exists private_office_step_events_delete_own
  on public.private_office_step_events;

drop trigger if exists private_office_step_matters_updated_at
  on public.private_office_step_matters;
create trigger private_office_step_matters_updated_at
  before update on public.private_office_step_matters
  for each row execute function public.set_private_office_updated_at();

create or replace function public.prevent_private_office_step_envelope_change()
returns trigger language plpgsql as $$
begin
  if new.owner_id is distinct from old.owner_id then
...
drop trigger if exists private_office_step_envelope_guard
  on public.private_office_step_matters;
create trigger private_office_step_envelope_guard
  before update on public.private_office_step_matters
  for each row execute function public.prevent_private_office_step_envelope_change();

create or replace function public.private_office_create_step_matter(
  p_id uuid,
  p_owner_id text,
  p_workflow_id text,
  p_state jsonb,
...
returns setof public.private_office_step_matters
language plpgsql
security invoker
set search_path = ''
as $$
declare
  created public.private_office_step_matters%rowtype;
begin
  if nullif(trim(p_owner_id), '') is null then
    raise exception 'owner id is required';
  end if;
  ...
  insert into public.private_office_step_matters (
  ...
  insert into public.private_office_step_events (
    matter_id, owner_id, event_type, actor_id, metadata
  )
  values (
    p_id,
...
create or replace function public.private_office_commit_step_state(
  p_matter_id uuid,
  p_owner_id text,
  p_expected_version integer,
  p_state jsonb,
...
declare
  current_workflow text;
  updated public.private_office_step_matters%rowtype;
begin
  select workflow_id
    into current_workflow
  from public.private_office_step_matters
  where id = p_matter_id
    and owner_id = p_owner_id
    and version = p_expected_version
  for update;
  ...
  update public.private_office_step_matters
  set
    version = p_expected_version + 1,
    state = p_state,
    updated_at = now()
...
revoke all on function public.private_office_create_step_matter(
  uuid, text, text, jsonb, text
) from public, anon, authenticated;
grant execute on function public.private_office_create_step_matter(
  uuid, text, text, jsonb, text
) to service_role;

revoke all on function public.private_office_commit_step_state(
  uuid, text, integer, jsonb, text, text, jsonb
) from public, anon, authenticated;
grant execute on function public.private_office_commit_step_state(
  uuid, text, integer, jsonb, text, text, jsonb
) to service_role;
