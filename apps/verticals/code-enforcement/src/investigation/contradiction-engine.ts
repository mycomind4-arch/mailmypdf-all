...
// Adapted from ruthlessinvestigator's contradiction detection.
// Pure functions — no server or AI provider dependency.

import type { Claim, Evidence, Contradiction, ContradictionStatus } from "./types";
...
export function detectPotentialContradictions(
    ...
    for (let j = i + 1; j < claimArray.length; j++) {
      ...
      if (aContradictsB || bContradictsA) {
        ...
        if (!seen.has(key)) {
          seen.add(key);
          contradictions.push({
            id: nextId("contradiction"), claimA: a.id, claimB: b.id,
            description: `Potential contradiction between "${truncate(a.text)}" and "${truncate(b.text)}"`,
            status: "POTENTIAL", detectedBy: "contradiction-engine", detectedAt: Date.now(),
          });
        }
      }
    }
  ...
  for (const ev of evidence.values()) {
    if (ev.supportsClaimId && ev.contradictsClaimId && ev.supportsClaimId !== ev.contradictsClaimId) {
      ...
      if (!seen.has(key) && claims.has(aId) && claims.has(bId)) {
        ...
        contradictions.push({
          id: nextId("contradiction"), claimA: aId, claimB: bId,
  ...
  for (let i = 0; i < evidenceArray.length; i++) {
    for (let j = i + 1; j < evidenceArray.length; j++) {
      ...
      if (ea.sourceId === eb.sourceId && ea.supportsClaimId && eb.contradictsClaimId && ea.supportsClaimId === eb.contradictsClaimId) {
        const key = `ev:${ea.id}::${eb.id}`;
        if (!seen.has(key)) {
          seen.add(key);
          contradictions.push({
            id: nextId("contradiction"), claimA: ea.id, claimB: eb.id,
...
export function investigateContradiction(
  ...
  if (!a || !b) return { status: "UNRESOLVED", resolution: "Could not retrieve both sides of the contradiction." };

  const aText = "text" in a ? a.text : "";
  const bText = "text" in b ? b.text : "";

  if (containsProjection(aText) && containsMeasurement(bText)) {
    return { status: "EXPLAINED", resolution: "Contradiction explained by difference between projected and observed values." };
  }
  ...
  if (aDate && bDate && aDate !== bDate) {
    return { status: "EXPLAINED", resolution: `Contradiction explained by temporal difference: one claim refers to ${aDate} while the other refers to ${bDate}.` };
  }
  return { status: "CONFIRMED", resolution: "Contradiction confirmed — the claims are genuinely in conflict and cannot be explained by differences in definition, time, or scope." };
}

export function runContradictionAnalysis(
  claims: Map<string, Claim>,
  evidence: Map<string, Evidence>
): Contradiction[] {
  const detected = detectPotentialContradictions(claims, evidence);
  return detected.map((c) => {
    const result = investigateContradiction(c, claims, evidence);
    return { ...c, status: result.status, resolution: result.resolution };
  });
}

function truncate(s: string, len = 80): string { return s.length > len ? s.substring(0, len) + "…" : s; }
function containsProjection(text: string): boolean { return /project|forecast|estimate|expect|anticipat|predict/i.test(text); }
function containsMeasurement(text: string): boolean { return /measur|observ|actual|record|report.*\d/i.test(text); }
function extractDate(text: string): string | null { const m = text.match(/\b(20\d{2})\b/); return m ? m[1] : null; }
