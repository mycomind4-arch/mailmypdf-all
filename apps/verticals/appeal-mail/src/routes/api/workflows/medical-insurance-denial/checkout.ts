...
export const Route = createFileRoute("/api/workflows/medical-insurance-denial/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "medical-insurance-denial") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Medical Insurance Denial Appeal Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "medical-insurance-denial",
            ...
            success_url: `${appUrl}/workflows/medical-insurance-denial?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/medical-insurance-denial?checkout=cancelled`,
          });
