...
export const Route = createFileRoute("/api/workflows/financial-aid-special-circumstances/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "financial-aid-special-circumstances") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const quote = calculateQuote({
            workflowId: "financial-aid-special-circumstances",
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Financial Aid Special Circumstances Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "financial-aid-special-circumstances",
            ...
            success_url: `${appUrl}/workflows/financial-aid-special-circumstances?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/financial-aid-special-circumstances?checkout=cancelled`,
          });
