...
import { calculateQuote, PRICES, LABELS } from "@mailmypdf/pricing";
    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "edd-denial") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const supportingPages = Math.max(0, Number(a.packet.supportingPageCount || 0));
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            success_url: `${appUrl}/workflows/edd-denial?checkout=success&session_id={CHECKOUT_SESSION_ID}`,
            cancel_url: `${appUrl}/workflows/edd-denial?checkout=cancelled`,
          });
