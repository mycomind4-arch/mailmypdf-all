...
export const Route = createFileRoute("/api/workflows/out-of-network-denial/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "out-of-network-denial") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Out-of-Network Denial Appeal Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "out-of-network-denial",
            ...
            success_url: `${appUrl}/workflows/out-of-network-denial?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/out-of-network-denial?checkout=cancelled`,
          });
