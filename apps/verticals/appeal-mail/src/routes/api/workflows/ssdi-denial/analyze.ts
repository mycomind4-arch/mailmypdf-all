      ...
      POST: async ({ request }) => {
        try {
          ...
          const decision = createDecision("claim_denial", {
            ...
            decisionTypeLabel: analysis.decision || "SSDI denial",
            ...
            issues: (analysis.issues || []).map((item, index) => ({
              id: `${index}-${crypto.randomUUID()}`,
              description: item.issue || "Issue identified in SSDI decision",
          ...
          const grounds = (analysis.issues || []).map((issue, index) =>
            createGround("factual_error", {
              ...
              claim: issue.issue || "Review a stated SSDI decision issue",
          ...
          const evidence = [
            createEvidence("document", "Original SSDI denial/decision", {
              documentId: document.id,
              documentFilename: document.filename,
              uploadedAt: new Date().toISOString(),
            }),
            ...(analysis.evidenceMentioned || []).map((label) =>
          ...
          if (grounds.length)
            grounds[0].supportingEvidenceIds = evidence.map((item) => item.id);
