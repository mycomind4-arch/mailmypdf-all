...
export const Route = createFileRoute("/api/workflows/ssdi-denial/draft")({server:{handlers:{ POST: async ({ request }) => { try {
  ...
  const workflow = getWorkflow("ssdi-denial"); const draftConfig = await resolveAI("ssdi-denial", "draft"); const validationConfig = await resolveAI("ssdi-denial", "validation"); const analysis = input.analysis || appeal.decision;
  const draft = input.draftOverride?.trim() || await callAIText(draftConfig, "Draft only from verified case facts. Do not invent medical diagnoses, functional limitations, law, SSA policy, dates, deadlines, or outcomes. Return only the response letter.", [`Create a response for the workflow: ${workflow.title}.`, workflow.workflowPrompt, `Focus on: ${workflow.focusAreas.join(", ")}.`, `CASE ANALYSIS:\n${JSON.stringify(analysis)}`].join("\n\n"));
  const validation = await callAIText(validationConfig, "Audit this SSDI appeal draft. Return strict JSON with valid, issues, unsupportedClaims, missingEvidence, suggestions. Flag unsupported medical claims, fabricated SSA/legal language, missing evidence, contradictions, deadline problems, and factual uncertainty.", [`CASE ANALYSIS:\n${JSON.stringify(analysis)}`, `DRAFT:\n${draft}`].join("\n\n"), true);
