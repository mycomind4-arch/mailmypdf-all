    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "ssi-denial") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            success_url: `${appUrl}/workflows/ssi-denial?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/ssi-denial?checkout=cancelled`,
          });
