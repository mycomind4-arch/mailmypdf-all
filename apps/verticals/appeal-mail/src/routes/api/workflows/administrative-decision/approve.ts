...
export const Route = createFileRoute("/api/workflows/administrative-decision/approve")({ server: { handlers: { POST: async ({ request }) => {
  try {
    ...
    const supabase=await getSupabaseServer(); const {data:appeal,error}=await supabase.from("appeals").select("*").eq("id",input.appealId).single(); if(error||!appeal)return Response.json({error:"Appeal case not found."},{status:404}); if(appeal.user_id!==user.id)return Response.json({error:"You do not own this appeal."},{status:403}); if(appeal.workflow_id!=="administrative-decision")return Response.json({error:"Workflow mismatch."},{status:409}); if(!appeal.draft?.trim())return Response.json({error:"Draft is required before approval."},{status:409});
    ...
    const packet={id:crypto.randomUUID(),recipientName:r.name,recipientAddress1:r.address1,recipientAddress2:r.address2,recipientCity:r.city,recipientState:r.state,recipientZip:r.zip,mailingMethod:input.mailingMethod,workflowId:"administrative-decision",finalDocumentType:"response-pdf",responseSheets:sresponseSheetsSafe(responseSheets),supportingSheets,pricing};
