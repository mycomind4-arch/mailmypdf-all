    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "claim-denial-letter") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const quote = calculateQuote({
            workflowId: "claim-denial-letter",
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Claim Denial Letter Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "claim-denial-letter",
            ...
            success_url: `${appUrl}/workflows/claim-denial-letter?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
