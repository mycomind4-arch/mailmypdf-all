...
export const Route = createFileRoute("/api/workflows/insurance-coverage-denial/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "insurance-coverage-denial") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "insurance-coverage-denial",
            ...
            success_url: `${appUrl}/workflows/insurance-coverage-denial?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
