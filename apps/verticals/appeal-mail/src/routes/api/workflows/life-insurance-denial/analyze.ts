...
export const Route = createFileRoute("/api/workflows/life-insurance-denial/analyze")({server:{handlers:{POST:async ({ request }) => {
  try {
    ...
    evidence.unshift(createEvidence("document", "Original life insurance denial", { documentId: document.id, documentFilename: document.filename, uploadedAt: new Date().toISOString(), groundIds })); if (evidence.length && grounds.length) grounds[0].supportingEvidenceIds = evidence.map((x) => x.id);
