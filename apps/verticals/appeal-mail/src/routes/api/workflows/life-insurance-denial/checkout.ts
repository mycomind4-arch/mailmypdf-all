...
import { calculateQuote, PRICES, LABELS, serializeQuote } from "@mailmypdf/pricing";

export const Route = createFileRoute("/api/workflows/life-insurance-denial/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "life-insurance-denial") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const responsePages = Math.max(1, Number(a.packet.responsePageCount || 1));
          const supportingPages = Math.max(0, Number(a.packet.supportingPageCount || Math.max(0, (a.evidence || []).length)));
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Life Insurance Denial Appeal Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "life-insurance-denial",
            ...
            success_url: `${appUrl}/workflows/life-insurance-denial?checkout=success&session_id={CHECKOUT_SESSION_ID}`,
            cancel_url: `${appUrl}/workflows/life-insurance-denial?checkout=cancelled`,
          });
