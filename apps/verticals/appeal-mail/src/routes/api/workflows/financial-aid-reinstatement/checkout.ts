...
export const Route = createFileRoute("/api/workflows/financial-aid-reinstatement/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "financial-aid-reinstatement") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const quote = calculateQuote({
            workflowId: "financial-aid-reinstatement",
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Financial Aid Reinstatement Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "financial-aid-reinstatement",
            ...
            success_url: `${appUrl}/workflows/financial-aid-reinstatement?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/financial-aid-reinstatement?checkout=cancelled`,
          });
