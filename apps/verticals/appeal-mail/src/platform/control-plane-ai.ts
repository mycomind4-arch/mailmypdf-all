...
export async function callAIText(cfg: AIConfig, system: string, user: string, json = false): Promise<string> {
...
export async function callAIWithDocument(cfg: AIConfig, system: string, user: string, document: { mimeType: string; base64: string }, json = false): Promise<string> {
  if (!document.base64 || document.base64.length > 28_000_000) throw new Error('Source document exceeds the AI input limit.');
  const base = (cfg.apiBaseUrl || (cfg.provider === 'gemini' ? 'https://generativelanguage.googleapis.com' : cfg.provider === 'openai' ? 'https://api.openai.com' : 'https://api.anthropic.com')).replace(/\/$/, '');
  let response: Response;
  if (cfg.provider === 'anthropic') {
  ...
  if (cfg.provider === 'openai') {
    response = await fetch(`${base}/v1/chat/completions`, { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${cfg.apiKey}` }, body: JSON.stringify({ model: cfg.model, temperature: 0.2, response_format: json ? { type: 'json_object' } : undefined, messages: [{ role: 'system', content: system }, { role: 'user', content: [{ type: 'text', text: user }, { type: 'image_url', image_url: { url: `data:${document.mimeType};base64,${document.base64}` } }] }] }) });
    if (!response.ok) throw new Error(`OpenAI request failed: ${response.status}`);
    const body = await response.json() as any;
    return body.choices?.[0]?.message?.content?.trim() ?? '';
  }
  response = await fetch(`${base}/v1beta/models/${encodeURIComponent(cfg.model)}:generateContent?key=${encodeURIComponent(cfg.apiKey)}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ systemInstruction: { parts: [{ text: system }] }, contents: [{ role: 'user', parts: [{ inlineData: { mimeType: document.mimeType, data: document.base64 } }, { text: user }] }], generationConfig: { temperature: 0.2, responseMimeType: json ? 'application/json' : 'text/plain' } }) });
  if (!response.ok) throw new Error(`Gemini request failed: ${response.status}`);
  const body = await response.json() as any;
  return body.candidates?.[0]?.content?.parts?.map((p:any)=>p.text || '').join('').trim() ?? '';
}

export function parseAIJson(text: string): any { const cleaned = text.trim().replace(/^```json\s*/i,'').replace(/```$/,'').trim(); try { return JSON.parse(cleaned); } catch { throw new Error('AI provider returned invalid JSON'); } }
...
export async function analyzeDeniedClaim(input: { documentText: string; facts: Record<string,string>; objective: string }) {
  ...
  const text = await callAIText(cfg, prompt, JSON.stringify({ workflow: 'denied-claim', documentText: input.documentText, facts: input.facts, objective: input.objective, output: { findings: [], facts: [], evidenceGaps: [], grounds: [], deadline: null, blockingIssues: [] } }), true);
...
export async function draftDeniedClaim(input: { analysis: unknown; workflowFacts: Record<string,string>; objective: string }) {
  ...
  const text = await callAIText(cfg, prompt, JSON.stringify(input));
...
export async function validateDeniedClaim(input: { analysis: unknown; draft: string }) {
  ...
  const text = await callAIText(cfg, prompt, JSON.stringify(input), true);
  return { provider: cfg.provider, model: cfg.model, validation: parseAIJson(text) };
}
