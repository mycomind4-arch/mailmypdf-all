...
export function MailStep({ matter, onComplete }: StepComponentProps) {
  ...
  const pricing = CAR_INSURANCE_APPEAL_PRICING;
  // car-insurance-appeal's pricing profile does not include mail in the
  // preparation fee (unlike the ADVANCED-band workflows) — the canonical
  // checkout quote (calculateQuote in @mailmypdf/pricing) charges the full
  // certified-mail price on top of the preparation fee, so this estimate
    ...
    return (
      <SectionCard title="Your documents are ready." headerAside={<StatusPill tone="success" label="Workflow complete" />}>
        <p style={{ color: "var(--wf-color-stone)" }}>
          Your Car Insurance Appeal packet has been generated and is now in the mailing queue.
        </p>
        ...
        <div style={{ marginTop: "1.25rem" }}>
          ...
          <ul style={{ marginTop: "0.5rem", display: "flex", flexDirection: "column", gap: "0.5rem", fontSize: "0.85rem", color: "var(--wf-color-stone)" }}>
            ...
            <li>If the appeal isn't resolved, explore next steps like a state insurance department complaint or independent appraisal/arbitration.</li>
          </ul>
        </div>
      </SectionCard>
    );
    ...
    >
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
        <div>
          ...
          <p style={{ fontSize: "0.9rem", margin: "0.25rem 0 0.75rem" }}>{intake.insurer ? `${intake.insurer} — Claims Department` : "—"}</p>
