...
export const onRequestPost = async ({ request, env }: { request: Request; env: Env }): Promise<Response> => {
  try {
    ...
    const queued = await queueMailJobExecution(env, paidIntent as { id: unknown; business_id: unknown; mail_job_id: unknown; workflow_id: unknown });
    ...
    console.error("[mail-response:smallbusiness] Error:", error);
    return json({ error: "Unable to verify mailing payment." }, 502);
  }
};
