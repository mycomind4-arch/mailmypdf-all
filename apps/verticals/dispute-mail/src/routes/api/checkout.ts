    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          // Map PricingKey to MailClass (standard → first_class)
          const mailClassMap: Record<PricingKey, MailClass> = {
            standard: "first_class",
            certified: "certified",
            registered: "registered",
          };
          const mailClass: MailClass = mailClassMap[method];
          if (!mailClass) {
            return Response.json(
              { error: "Invalid mailing method mapping." },
              { status: 500 }
            );
          }
          ...
          if (input?.caseId) {
            const { data: ownedCase } = await supabase
              ...
              .maybeSingle();
            if (!ownedCase) {
              return Response.json({ error: "Dispute case not found." }, { status: 404 });
            }
          }
          ...
          const { data: intent, error: intentError } = await supabase
            ...
            .insert({
              ...
              status: "draft",
              mailing_method: mailClass,
              draft,
              recipient,
              matter_reference: input?.matterReference?.trim() || workflowId,
              matter_type: input?.matterType?.trim() || "dispute-mail",
          ...
          try {
            ...
            await supabase
              ...
              .update({
                status: "failed",
                error_message: error instanceof Error ? error.message : "Stripe checkout creation failed",
              })
              .eq("id", intent.id)
              .eq("owner_id", user.id);
            throw error;
