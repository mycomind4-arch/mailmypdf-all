...
function selectedCategories(input:Record<string,unknown>):string[]{const raw=input.categories;if(!Array.isArray(raw))return [...INTERNAL_AFFAIRS_RECORD_CATEGORIES];const known=new Set(INTERNAL_AFFAIRS_RECORD_CATEGORIES);const selected=raw.filter((entry):entry is string=>typeof entry==='string'&&known.has(entry as typeof INTERNAL_AFFAIRS_RECORD_CATEGORIES[number]));return selected.length?selected:[...INTERNAL_AFFAIRS_RECORD_CATEGORIES]}
...
function scopeText(input:Record<string,unknown>):string{
  ...
  const start=text(input,'dateStart');const end=text(input,'dateEnd')
...
function validateInternalAffairs(request:ValidatedRequest):readonly {field:string;message:string}[]{const issues:{field:string;message:string}[]=[];const corpus=request.items.map(item=>item.description.toLowerCase()).join(' ');if(!corpus.includes('complaint/allegation description:'))issues.push({field:'allegation',message:'Describe the complaint, conduct, incident, or policy issue in plain language.'});if(!corpus.includes('cover '))issues.push({field:'dateRange',message:'Provide a beginning and ending date for the complaint or investigation period.'});if(!corpus.includes('complaint/ia case number')&&!corpus.includes('related incident/report number')&&!corpus.includes('officer/employee identifiers')&&!corpus.includes('complainant/involved person'))issues.push({field:'identifiers',message:'Provide at least one complaint/case number, related incident number, involved person, or officer/employee identifier.'});return issues}

export function buildInternalAffairsRecordsRequest(input:Record<string,unknown>){
  const complaintNumber=text(input,'complaintNumber');const incidentNumber=text(input,'incidentNumber');const person=text(input,'person');const allegation=text(input,'allegation');const start=text(input,'dateStart');const end=text(input,'dateEnd');const categories=selectedCategories(input)
  return {title:`Internal Affairs Records — ${complaintNumber??incidentNumber??person??'Complaint'}`,agency:text(input,'agency')??'',jurisdiction:text(input,'jurisdiction'),purpose:text(input,'purpose')??'Identify, preserve, obtain, and compare complaint, investigation, evidence-index, findings, review, policy, and withholding records associated with the specified internal-affairs or professional-standards matter.',scope:JSON.stringify({workflow:'internal-affairs-records',complaintNumber,incidentNumber,dateStart:start,dateEnd:end,person,officerNames:text(input,'officerNames'),department:text(input,'department'),allegation}),items:categories.map(category=>({category,description:describe(category,input),dateStart:start,dateEnd:end,custodian:text(input,'department'),systemHint:category==='complaint-intake-and-tracking'||category==='internal-affairs-investigation-records'||category==='professional-standards-review-records'?'internal affairs / professional standards case-management system':category==='evidence-and-media-indexes'?'digital evidence / document management system':undefined,format:category==='complaint-intake-and-tracking'||category==='evidence-and-media-indexes'?'native export, CSV, JSON, or other structured format where maintained':undefined}))}
}
