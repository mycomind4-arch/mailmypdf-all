...
export const SEARCH_WARRANT_INTAKE = [
  { id: 'agency', label: 'Agency or records custodian', required: true, helpText: 'Executing law-enforcement agency, court clerk, prosecutor, or other likely custodian.' },
  { id: 'warrantNumber', label: 'Warrant / application number', helpText: 'Warrant, application, miscellaneous, docket, or tracking number when known.' },
  { id: 'caseNumber', label: 'Related court / criminal case number', helpText: 'Related criminal, civil, miscellaneous, or court case number when known.' },
  { id: 'dateStart', label: 'Beginning date', required: true, helpText: 'Beginning of the relevant application/execution period.' },
  { id: 'dateEnd', label: 'Ending date', required: true, helpText: 'End of the relevant application/execution period.' },
  { id: 'location', label: 'Search location / property', helpText: 'Address, parcel, vehicle, device, account, or other searched place/property when known.' },
  { id: 'subject', label: 'Person / account / device subject', helpText: 'Name or other identifier associated with the warrant.' },
  { id: 'executingAgency', label: 'Executing agency', helpText: 'Agency or task force that applied for or executed the warrant.' },
  { id: 'incidentNumber', label: 'Related incident / report number', helpText: 'Police report, CAD, investigation, evidence, or incident identifier.' },
] as const
...
function selectedCategories(input: Record<string, unknown>): string[] {
  ...
  const selected = raw.filter(
    (entry): entry is string => typeof entry === 'string' && known.has(entry as typeof SEARCH_WARRANT_RECORD_CATEGORIES[number]),
  )
...
function describe(category: string, input: Record<string, unknown>): string {
  ...
  const descriptions: Record<string, string> = {
    'warrant-and-order': `The search warrant, signed warrant, judicial order, authorization, amendments, extensions, corrected warrants, and records sufficient to identify the issuing court/judicial officer, issuance date, authorized scope, and return deadline, to the extent unsealed and disclosable.${scope}`,
    'application-affidavit-and-probable-cause-materials': `Search-warrant application, affidavit, declaration, probable-cause statement, incorporated statement of facts, supplemental affidavit, and comparable supporting materials, to the extent unsealed and disclosable; request segregable portions where only part is restricted.${scope}`,
    'attachments-exhibits-and-incorporated-materials': `Attachments, exhibits, photographs, maps, lists, incorporated documents, attachments describing items to be seized or places/accounts/devices to be searched, and referenced supporting materials associated with the warrant application or order.${scope}`,
    'return-and-inventory': `Executed warrant return, return of service, inventory, receipt, judicial return filing, property list, seizure list, and records sufficient to show when and how the warrant was returned to the issuing court or authority.${scope}`,
    'execution-and-service-records': `Execution logs, service records, operational chronology, officer/unit assignments, entry/service time records, execution reports, after-action records, and records sufficient to identify when, where, and by whom the warrant was executed, to the extent maintained and disclosable.${scope}`,
    'evidence-property-and-seizure-records': `Evidence/property receipts, seizure inventories, evidence tags, chain-of-custody or property-control records, transfer records, booking/property-system entries, and records sufficient to identify property or data seized pursuant to the warrant.${scope}`,
    'docket-index-and-case-tracking': `Docket entries, register-of-actions entries, warrant/application indexes, case-tracking records, filing history, event codes, document lists, and structured metadata sufficient to identify filings, sealing status, return filings, and related proceedings.${scope}`,
    'sealing-unsealing-and-access-orders': `Orders or docket records concerning sealing, partial sealing, unsealing, delayed disclosure, access restrictions, redactions, expiration of sealing, and records sufficient to identify the current access status and legal basis for any restriction.${scope}`,
    ...
    'retention-redaction-and-withholding-records': `Retention classifications, preservation holds, destruction/deletion records, redaction logs, withholding determinations, exemption records, sealing references, privilege logs where maintained, and records stating the basis for responsive material not produced.${scope}`,
  }
  return descriptions[category] ?? `Records concerning ${category}.${scope}`
}
...
export function buildSearchWarrantRecordsRequest(input: Record<string, unknown>) {
  ...
  return {
    ...
    purpose: text(input, 'purpose') ?? 'Identify, preserve, obtain, and compare unsealed or otherwise lawfully disclosable search-warrant papers, supporting materials, returns, inventories, execution records, evidence indexes, docket records, and withholding/sealing evidence.',
        ...
        category === 'docket-index-and-case-tracking' || category === 'sealing-unsealing-and-access-orders'
          ...
          : category === 'evidence-property-and-seizure-records'
            ...
            : category === 'execution-and-service-records'
              ...
              : undefined,
      format: category === 'docket-index-and-case-tracking'
...
export const searchWarrantRecordsWorkflow: RecordsWorkflow = createRecordsWorkflow({
  ...
  description: 'Build a targeted request for search-warrant papers, application/affidavit materials, returns and inventories, execution records, evidence/property records, docket/index data, sealing status, related report/media indexes, and withholding evidence.',
  searchIntent: 'search warrant records request',
  seo: {
    title: 'Search Warrant Records Request — Warrant, Affidavit, Return & Inventory',
    description: 'Request unsealed or lawfully disclosable search-warrant records, affidavits, supporting exhibits, returns, inventories, execution records, docket entries, sealing orders, and withholding records.',
  ...
  policies: [
    {
      ...
      version: '1.0.0',
      rules: {
        doNotAssumeWarrantAffidavitsArePublic: true,
        doNotCircumventSealingOrders: true,
  ...
  responseAnalysis: {
    ...
    async analyze(input: unknown) {
      if (!input || typeof input !== 'object') throw new Error('SEARCH_WARRANT_PRODUCTION_ANALYSIS_INPUT_INVALID')
