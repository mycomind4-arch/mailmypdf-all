...
export default function WorkflowStartPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!workflow) {
    return <main className="container" style={{ maxWidth: 860, padding: '4rem 1.5rem 6rem' }}><h1>Workflow not found</h1><Link href="/workflows">← Back to workflows</Link></main>
  }
  ...
  return (
    <main className="container" style={{ maxWidth: 860, padding: '4rem 1.5rem 6rem' }}>
      ...
      <section className="card" style={{ marginTop: 32, padding: 28 }}>
        ...
        <p style={{ color: '#94a3b8', lineHeight: 1.6 }}>The workflow preserves the selected workflow ID and analyzes the source document before drafting.</p>
      ...
      {analysis && <section className="card" style={{ marginTop: 18, padding: 28 }}>
        ...
        <p style={{ marginTop: 16, fontSize: 13, color: '#94a3b8' }}>The next stage should use these extracted facts to build the draft and review workflow for this same workflow ID.</p>
      </section>}
      ...
      <div style={{ marginTop: 20, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        {analysis && <button onClick={() => router.push(`/workflows/${id}/start?analyzed=1`)} style={{ display: 'inline-flex', alignItems: 'center', padding: '0.8rem 1.15rem', borderRadius: '0.55rem', background: 'transparent', border: '1px solid rgba(167,139,250,.35)', color: '#c4b5fd', fontWeight: 600 }}>Continue this workflow</button>}
      </div>
    </main>
  )
}
