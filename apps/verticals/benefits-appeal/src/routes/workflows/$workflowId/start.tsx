...
function WorkflowStart() {
  ...
  if (!workflow.executable) return <main className='container section'><h1>{workflow.title}</h1><p className='muted'>This workflow is in the public catalog but is not yet executable.</p><Link to='/workflows'>Back to workflows</Link></main>
  ...
  if (!user) return <main className='container section'><div className='card' style={{ padding: 32 }}><div className='eyebrow'>Account required</div><h1 style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 54 }}>Sign in to start {workflow.title}.</h1><p className='muted' style={{ lineHeight: 1.7 }}>The workflow itself is private. Your source documents and case information will be associated with your account.</p><Link className='btn btn-primary' style={{ marginTop: 18 }} to='/auth' search={{ returnTo: `/workflows/${workflow.slug}/start` }}>Sign in or create an account →</Link></div></main>
  ...
  async function analyze() {
    ...
    try {
      ...
      const response = await fetch(`/api/workflows/${encodeURIComponent(workflow.slug)}/analyze`, { method: 'POST', headers: { Authorization: `Bearer ${accessToken}` }, body: form })
