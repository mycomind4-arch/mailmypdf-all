...
function ProtectedContent() {
  ...
  const isWorkflowHub = path === '/workflows' || path === '/workflows/'
...
function Gate({ loading = false }: { loading?: boolean }) {
  ...
  return <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24 }}><section className='card' style={{ maxWidth: 620, width: '100%', padding: 40, textAlign: 'center' }}><div className='eyebrow'>MailMyPDF Account</div><h1 style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 48, margin: '16px 0 8px' }}>{loading ? 'Loading your account…' : 'Sign in to start this workflow'}</h1><p className='muted' style={{ lineHeight: 1.7 }}>{loading ? 'Checking your account session.' : 'Workflow intake, uploaded documents, analysis, drafts, and mailing records are private to your account.'}</p>{!loading && <Link className='btn btn-primary' style={{ marginTop: 24 }} to={`/auth?returnTo=${returnTo}` as never}>Sign in or create an account →</Link>}</section></main>
}
