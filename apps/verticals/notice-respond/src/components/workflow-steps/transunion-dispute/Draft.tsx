...
import { generateTransUnionDisputeDraft, type TransUnionDisputeIntake } from "@/domain/step-workflows/transunion-dispute";
...
export function DraftStep({ matter, onComplete }: StepComponentProps) {
  ...
  const letter = generateTransUnionDisputeDraft(matter);

  return (
    <>
      <SectionCard
        title="Draft dispute letter"
        description="We assembled your FCRA dispute letter from your intake and analysis. Review every account name, number, and explanation carefully — this is what gets mailed to TransUnion."
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.9rem" }}>
            {[
              { label: "Recipient", value: "TransUnion LLC — Consumer Dispute Center, P.O. Box 2000, Chester, PA 19016" },
      ...
      <SectionCard title="Draft quality check" description="We've reviewed your draft for the elements a TransUnion reinvestigation letter needs.">
        <DataTable
          columns={[
