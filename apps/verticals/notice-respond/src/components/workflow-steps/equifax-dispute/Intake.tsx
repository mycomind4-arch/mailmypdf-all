...
export function IntakeStep({ matter, onUpdateData, onComplete, goToStep }: StepComponentProps) {
  ...
  const [form, setForm] = useState<EquifaxDisputeIntake>(intake);
  ...
  function set<K extends keyof EquifaxDisputeIntake>(key: K, value: EquifaxDisputeIntake[K]) {
  ...
  return (
    <>
      <SectionCard
        ...
        description="This comes from the top of your Equifax credit report and identifies you to Equifax. Use the address you want Equifax to write back to."
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
          ...
          <Field label="Report or confirmation number" hint="If your report shows one.">
            <TextField value={form.reportNumber ?? ""} placeholder="e.g. EQ-2026-0043821" onChange={(e) => set("reportNumber", e.target.value)} />
          </Field>
        </div>
      </SectionCard>
      ...
      >
        <Field label="Additional context">
          <TextArea rows={2} value={form.additionalContext ?? ""} placeholder="Anything Equifax should know that isn't covered by the items above." onChange={(e) => set("additionalContext", e.target.value)} />
        </Field>
        <div style={{ marginTop: "1rem" }}>
          <Field label="What do you want Equifax to do?" hint="Leave blank to use the default request to investigate and correct or delete each item.">
