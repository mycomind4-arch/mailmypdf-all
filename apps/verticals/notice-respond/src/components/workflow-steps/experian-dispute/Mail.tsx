...
import { EXPERIAN_DISPUTE_PRICING } from "@/domain/step-workflows/experian-dispute";

const MAILING_CHECKLIST = [
  { label: "Experian National Consumer Assistance Center address confirmed", ready: true },
...
export function MailStep({ onComplete, matter }: StepComponentProps) {
  ...
  const pricing = EXPERIAN_DISPUTE_PRICING;
  ...
  if (isSent) {
    return (
      <SectionCard title="Your dispute letter is ready." headerAside={<StatusPill tone="success" label="Workflow complete" />}>
        <p style={{ color: "var(--wf-color-stone)" }}>Your Experian dispute packet has been generated and is now in the mailing queue.</p>
        ...
        <div style={{ marginTop: "1.25rem" }}>
          ...
          <ul style={{ marginTop: "0.5rem", display: "flex", flexDirection: "column", gap: "0.5rem", fontSize: "0.85rem", color: "var(--wf-color-stone)" }}>
            <li>Experian has 30 days from receipt to investigate (45 if you send more information during that period) — mark your calendar.</li>
            ...
            <li>If Experian doesn't respond or the item stays wrong after investigation, you can escalate with a follow-up letter, a CFPB complaint, or a consumer law attorney.</li>
          </ul>
        </div>
      </SectionCard>
    );
  ...
  return (
    <SectionCard
      ...
      description="Your dispute letter is finalized. Confirm the recipient and delivery method before sending — certified mail starts the FCRA 30-day clock with proof of the date Experian received it."
    ...
    >
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
        <div>
          ...
          <p style={{ fontSize: "0.9rem", margin: "0.25rem 0 0.75rem" }}>Experian — National Consumer Assistance Center, P.O. Box 4500, Allen, TX 75013</p>
      ...
      <div style={{ marginTop: "1.25rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
        <CheckboxField label="I confirm the Experian mailing address is correct" checked={confirmed.address} onChange={(v) => setConfirmed((c) => ({ ...c, address: v }))} />
