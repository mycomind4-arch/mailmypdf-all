...
export function IntakeStep({ matter, onUpdateData, onComplete, goToStep }: StepComponentProps) {
  ...
  const [form, setForm] = useState<ExperianDisputeIntake>(intake);
  ...
  function set<K extends keyof ExperianDisputeIntake>(key: K, value: ExperianDisputeIntake[K]) {
  ...
  return (
    <>
      <SectionCard
        ...
        description="This comes from the top of your Experian credit report and identifies you to Experian's National Consumer Assistance Center. Use the address you want Experian to write back to."
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
          ...
          <Field label="Report or confirmation number" hint="If your report shows one.">
            <TextField value={form.reportNumber ?? ""} placeholder="e.g. EX-2026-0043821" onChange={(e) => set("reportNumber", e.target.value)} />
          </Field>
        </div>
      </SectionCard>
      ...
      >
        <Field label="Additional context">
          <TextArea rows={2} value={form.additionalContext ?? ""} placeholder="Anything Experian should know that isn't covered by the items above." onChange={(e) => set("additionalContext", e.target.value)} />
        </Field>
        <div style={{ marginTop: "1rem" }}>
          <Field label="What do you want Experian to do?" hint="Leave blank to use the default request to investigate and correct or delete each item.">
