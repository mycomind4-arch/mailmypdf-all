...
import { generateExperianDisputeDraft, type ExperianDisputeIntake } from "@/domain/step-workflows/experian-dispute";
...
export function DraftStep({ matter, onComplete }: StepComponentProps) {
  ...
  const letter = generateExperianDisputeDraft(matter);

  return (
    <>
      <SectionCard
        title="Draft dispute letter"
        description="We assembled your FCRA dispute letter from your intake and analysis. Review every account name, number, and explanation carefully — this is what gets mailed to Experian."
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.9rem" }}>
            {[
              { label: "Recipient", value: "Experian — National Consumer Assistance Center, P.O. Box 4500, Allen, TX 75013" },
      ...
      <SectionCard title="Draft quality check" description="We've reviewed your draft for the elements an Experian reinvestigation letter needs.">
        <DataTable
          columns={[
