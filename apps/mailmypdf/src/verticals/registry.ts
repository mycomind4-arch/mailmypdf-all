...
const appealReply = makeVertical({ id: "appeal-reply", slug: "appeal-reply", name: "AppealReply", shortName: "AppealReply", tagline: "Build and mail an appeal.", description: "Organize a decision, supporting documents, deadlines, and evidence into a response you can review before mailing.", category: "appeals", status: "live", executionState: "catalog", route: "/appeal-reply", icon: "Scale", primaryCTA: "Start an Appeal", enabled: true, capabilities: { requiresAI: true, requiresDocuments: true, supportsDrafting: true, supportsEvidence: true, supportsMailing: true } });
const noticeResponse = makeVertical({ id: "notice-response", slug: "notice-response", name: "NoticeResponse", shortName: "NoticeResponse", tagline: "Respond to notices before deadlines.", description: "Understand an official notice, organize the timeline, prepare your response, and mail it before the stated deadline.", category: "government", status: "live", executionState: "catalog", route: "/notice-response", icon: "Clock", primaryCTA: "Start a Response", enabled: true, capabilities: { requiresAI: true, requiresDocuments: true, supportsDrafting: true, supportsEvidence: false, supportsMailing: true } });
...
const smallBusinessMail = makeVertical({ id: "small-business-mail", slug: "small-business-mail", name: "Small Business Mail", shortName: "Business Mail", tagline: "Business correspondence, sent on time.", description: "Create, schedule, track, and prove the documents your business sends with team approvals, templates, and a permanent proof archive.", category: "business", status: "live", executionState: "catalog", route: "/small-business-mail", icon: "Building2", primaryCTA: "Start a Mailing", enabled: true, capabilities: { requiresAI: false, requiresDocuments: true, supportsDrafting: true, supportsEvidence: false, supportsMailing: true } });

export const verticals: VerticalDefinition[] = [disputeMail, appealReply, noticeResponse, claimProof, tenantReply, permitReply, benefitsAppeal, debtDefense, recordsRequest, privateOffice, smallBusinessMail];
...
export function getVerticalsByCategoryForNav(): Array<{ category: VerticalCategory; verticals: VerticalDefinition[] }> { const categories: VerticalCategory[] = ["government", "appeals", "disputes", "housing", "professional", "business"]; return categories.map((category) => ({ category, verticals: getNavigationVerticals().filter((v) => v.category === category) })).filter((group) => group.verticals.length > 0); }
