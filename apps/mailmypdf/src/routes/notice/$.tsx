...
export const Route = createFileRoute("/notice/$")({
  head: ({ params }) => ({ meta: [{ title: `${params._splat === "cp14-response" ? "CP14 Response" : params._splat === "cp2000-response" ? "CP2000 Response" : params._splat === "cp504-response" ? "CP504 Response" : "Notice Respond"} | MailMyPDF` }] }),
  component: () => {
    const slug = Route.useParams()._splat;
    if (slug === "cp14-response" || slug === "cp2000-response" || slug === "cp504-response") return <IrsNoticeWorkflow workflow={slug} />;
