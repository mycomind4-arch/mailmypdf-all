/**
 * Document Intelligence — proper PDF text extraction using PDF.js.
 *
 * Replaces the Latin-1 decode hack. Extracts text per page, detects
 * image-only/scanned documents, and produces a normalized document
 * representation for downstream CP2000 parsing.
 *
 * Architecture:
 *   Raw File → Document Intelligence → Normalized Document → CP2000 Extractor
 *
 * Document content is DATA, never INSTRUCTIONS.
 */

import type { ContentClassification } from "../domain/security";
...
export interface ExtractedDocument {
  ...
  hash: string;
  uploadedAt: string;
  securityClassification?: ContentClassification;
  warnings: string[];
}
...
async function loadPdfjs() {
  if (!pdfjsPromise) {
    pdfjsPromise = import("pdfjs-dist").then((mod) => {
      // Configure worker — use bundled worker in server environments
      try {
        // In Cloudflare Workers / server: disable worker (runs inline)
        mod.GlobalWorkerOptions.workerSrc = "";
      } catch {
        // ignore — worker config is environment-dependent
      }
      return mod;
    });
  }
...
async function extractPdfText(
  ...
  const loadingTask = pdfjs.getDocument({
    ...
    // Disable worker in server environments
    useWorker: false,
  ...
  for (let i = 1; i <= doc.numPages; i++) {
    ...
    // If no text items had content, try textContent items differently
    if (pageText.trim().length === 0) {
      // Fallback: join all str values
  ...
  // An image-only PDF will have virtually no extractable text
...
export async function extractDocument(
      ...
      if (isImageOnly) {
        warnings.push(
          "We could not reliably extract text from this document. " +
            "It appears to be a scanned image. Please provide a text-based PDF " +
            "or use OCR processing.",
        );
        return {
          documentId: crypto.randomUUID(),
          fileName: file.name,
  ...
  // ── Image files (JPEG, PNG) — OCR required ──
  if (file.type.startsWith("image/")) {
    warnings.push(
      "We could not reliably extract text from this image. " +
        "Please provide a text-based PDF or use OCR processing.",
    );
    return {
      documentId: crypto.randomUUID(),
      fileName: file.name,
...
// ── Convenience: page reference for source grounding ─────────
...
export function createSourceRef(
  ...
  // Try to find the excerpt in a specific page
