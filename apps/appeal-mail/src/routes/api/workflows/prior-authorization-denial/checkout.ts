...
export const Route = createFileRoute("/api/workflows/prior-authorization-denial/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "prior-authorization-denial") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const quote = calculateQuote({
            workflowId: "prior-authorization-denial",
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Prior Authorization Denial Appeal Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "prior-authorization-denial",
            ...
            success_url: `${appUrl}/workflows/prior-authorization-denial?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/prior-authorization-denial?checkout=cancelled`,
          });
