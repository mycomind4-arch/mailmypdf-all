    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "financial-aid-appeal") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const quote = calculateQuote({
            workflowId: "financial-aid-appeal",
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Financial Aid Appeal Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "financial-aid-appeal",
            ...
            success_url: `${appUrl}/workflows/financial-aid-appeal?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/financial-aid-appeal?checkout=cancelled`,
          });
