...
export const Route = createFileRoute("/api/workflows/financial-aid-suspension-appeal/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "financial-aid-suspension-appeal") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const quote = calculateQuote({
            workflowId: "financial-aid-suspension-appeal",
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Financial Aid Suspension Appeal Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "financial-aid-suspension-appeal",
            ...
            success_url: `${appUrl}/workflows/financial-aid-suspension-appeal?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/financial-aid-suspension-appeal?checkout=cancelled`,
          });
