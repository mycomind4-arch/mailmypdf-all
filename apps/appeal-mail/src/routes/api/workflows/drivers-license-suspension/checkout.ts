...
export const Route = createFileRoute("/api/workflows/drivers-license-suspension/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "drivers-license-suspension") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const quote = calculateQuote({
            workflowId: "drivers-license-suspension",
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Driver's License Suspension Response",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "drivers-license-suspension",
            ...
            success_url: `${appUrl}/workflows/drivers-license-suspension?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/drivers-license-suspension?checkout=cancelled`,
          });
