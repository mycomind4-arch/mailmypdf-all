    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "unemployment-denial") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Unemployment Denial Appeal Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "unemployment-denial",
            ...
            success_url: `${appUrl}/workflows/unemployment-denial?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
