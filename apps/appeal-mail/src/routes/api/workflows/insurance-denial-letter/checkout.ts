...
export const Route = createFileRoute("/api/workflows/insurance-denial-letter/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "insurance-denial-letter") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const quote = calculateQuote({
            workflowId: "insurance-denial-letter",
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Insurance Denial Letter Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "insurance-denial-letter",
            ...
            success_url: `${appUrl}/workflows/insurance-denial-letter?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/insurance-denial-letter?checkout=cancelled`,
          });
