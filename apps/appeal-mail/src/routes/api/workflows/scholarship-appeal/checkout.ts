...
export const Route = createFileRoute("/api/workflows/scholarship-appeal/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "scholarship-appeal") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Scholarship Appeal Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "scholarship-appeal",
            ...
            success_url: `${appUrl}/workflows/scholarship-appeal?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/scholarship-appeal?checkout=cancelled`,
          });
