...
export const Route = createFileRoute("/api/workflows/license-revocation-appeal/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "license-revocation-appeal") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const quote = calculateQuote({
            workflowId: "license-revocation-appeal",
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "License Revocation Appeal Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "license-revocation-appeal",
            ...
            success_url: `${appUrl}/workflows/license-revocation-appeal?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/license-revocation-appeal?checkout=cancelled`,
          });
