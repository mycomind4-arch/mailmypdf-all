    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "medicaid-denial") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Medicaid Denial Appeal Packet",
            ...
            success_url: `${appUrl}/workflows/medicaid-denial?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
