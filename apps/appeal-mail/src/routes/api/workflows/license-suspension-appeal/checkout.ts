...
export const Route = createFileRoute("/api/workflows/license-suspension-appeal/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "license-suspension-appeal") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const quote = calculateQuote({
            workflowId: "license-suspension-appeal",
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "License Suspension Appeal Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "license-suspension-appeal",
            ...
            success_url: `${appUrl}/workflows/license-suspension-appeal?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/license-suspension-appeal?checkout=cancelled`,
          });
