...
export const Route = createFileRoute("/api/workflows/sap-appeal/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            success_url: `${appUrl}/workflows/sap-appeal?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/sap-appeal?checkout=cancelled`,
          });
