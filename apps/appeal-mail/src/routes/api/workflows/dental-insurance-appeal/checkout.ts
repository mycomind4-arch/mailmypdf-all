...
export const Route = createFileRoute("/api/workflows/dental-insurance-appeal/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (a.workflow_id !== "dental-insurance-appeal") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
          ...
          const quote = calculateQuote({
            workflowId: "dental-insurance-appeal",
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            line_items: [{
              price_data: {
                currency: "usd",
                product_data: {
                  name: "Dental Insurance Appeal Packet",
            ...
            metadata: {
              appeal_id: a.id,
              workflow_id: "dental-insurance-appeal",
            ...
            success_url: `${appUrl}/workflows/dental-insurance-appeal?checkout=success&session_id=${encodeURIComponent("{CHECKOUT_SESSION_ID}")}`,
            cancel_url: `${appUrl}/workflows/dental-insurance-appeal?checkout=cancelled`,
          });
