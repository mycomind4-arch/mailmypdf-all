...
export function InsuranceClaimDenialPricing() {
  const profile = getWorkflowPricingProfile("denied-claim");
  ...
  return <section className="mx-auto max-w-6xl px-6 py-10">
    <div className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm leading-6 text-slate-700">
        ...
        <p className="mt-2">The exact total is calculated from the approved physical packet before payment. Extra pages are charged at the canonical rate when they exceed the included page count.</p>
      </div>
    </div>
  </section>;
}
