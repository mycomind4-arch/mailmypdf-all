# HANDOFF: NEXT AGENT — Benefits Appeal / MailMyPDF Platform
...
| Service | URL | Status |
|---------|-----|--------|
| Benefits Appeal (vertical app) | https://benefits-appeal.pages.dev | LIVE |
...
|------|----------|----------------|
| Benefits Appeal | `00fc98f5de295e96d2d7fd33020178fc8d73ddbf` | Fix FormData Content-Type in MailMyPDF platform client |
...
|-------|-------|------|------|
| Benefits Appeal (`node --import tsx --test tests/*.test.ts`) | 750 | 750 | 0 |
...
│                                                               │
│  ┌─────────────────┐     ┌─────────────────────────────────┐ │
│  │   Benefits Appeal    │     │    MailMyPDF Application         │ │
...
- **Benefits Appeal** (vertical): Owns appeal workflow logic, domain rules, readiness gates, Stripe checkout, and client UI. Deployed as a Cloudflare Pages app.
...
- Benefits Appeal uses `requireAuthenticatedUser(request)` in every protected route — extracts and validates the Supabase JWT from the `Authorization: Bearer` header.
- Admin authorization: `user_roles` table with RLS (service-role only). Roles: `customer`, `admin`, `super_admin`.
- No duplicate auth systems exist. Benefits Appeal does NOT create its own identity system.
...
- **Schema:** Defined in `benefits-appeal/supabase/schema.sql` (Benefits Appeal tables) and MailMyPDF migrations.
...
- **Control plane:** Self-hosted at `https://benefits-appeal.pages.dev/api/control-plane/ai`
...
- **Boundary:** MailMyPDF Application owns document storage. Benefits Appeal calls `POST {MAILMYPDF_API_URL}/v1/documents` with `Bearer` auth and multipart form data.
...
**Critical fix applied:** The `request()` helper in `benefits-appeal/src/platform/mailmypdf.ts` was setting `Content-Type: application/json` for FormData bodies, breaking multipart uploads. Fixed by only setting Content-Type for non-FormData bodies.

## 14. Communications Architecture

- **Boundary:** MailMyPDF Application owns communications/mailing. Benefits Appeal calls `POST {MAILMYPDF_API_URL}/v1/communications`.
- **Flow:** Benefits Appeal approve endpoint → readiness review → packet assembly → (after Stripe payment) → communication created → Lob submission → tracking number returned.
...
### Benefits Appeal (Cloudflare Pages secrets)
...
2. **No known schema drift.** Benefits Appeal schema in `supabase/schema.sql` matches deployed tables. MailMyPDF migrations are current.
...
3. **Email (Resend) is not configured in Benefits Appeal.** Transactional emails (receipts, status updates) are not sent. The MailMyPDF Application has Resend env vars but they may not be set in the deployment environment.
...
- Benefits Appeal is a product within MailMyPDF.
...
- Benefits Appeal owns appeal workflow/domain logic.
...
| Tests | VERIFIED | Benefits Appeal 750/750, MailMyPDF 492/493 (1 pre-existing) |
