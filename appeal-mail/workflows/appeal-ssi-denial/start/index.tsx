...
export type ClaimantFacts = {
  ...
  incomeFacts: string;
  resourceFacts: string;
  livingArrangementFacts: string;
...
const EMPTY_FACTS: ClaimantFacts = {
  ...
  incomeFacts: "",
  resourceFacts: "",
  livingArrangementFacts: "",
  eligibilityFacts: "",
  additionalFacts: "",
  requestedOutcome: "Reconsider the SSI denial using the submitted information and evidence.",
};
...
function currentDecisionBasis(analysis: WorkflowAnalysis | null): SsiDecisionBasis {
...
function formLabel(kind: string | null): string {
  return SSI_REQUIRED_FORMS.find((form) => form.kind === kind)?.label ??
    SSI_EVIDENCE_KINDS.find(([value]) => value === kind)?.[1] ??
    kind ??
    "Supporting document";
}

export default function SsiDenialWorkflow() {
  ...
  const [evidenceKind, setEvidenceKind] = useState<SsiEvidenceKind>("medical_records");
  ...
  const analysisReady = Boolean(
    analysis &&
      isSsiReconsiderationStage(appealStage) &&
      isSupportedSsiDecisionBasis(decisionBasis),
  );
  const requiredForms = requiredSsiFormsForBasis(decisionBasis);
  const formsReady = hasRequiredSsiForms(documents, decisionBasis);
  ...
  async function restore(id: string) {
    ...
    try {
      ...
      const inferred = ssiCompletedSteps({
        ...
        hasReconsiderationAnalysis: Boolean(
          storedAnalysis &&
            isSsiReconsiderationStage(currentAppealStage(storedAnalysis)) &&
            isSupportedSsiDecisionBasis(currentDecisionBasis(storedAnalysis)),
        ),
        hasClaimantFacts: Boolean(storedInput),
        hasDraft: Boolean(storedDraft?.bodyText),
        hasRequiredForms: hasRequiredSsiForms(snapshot.documents, currentDecisionBasis(storedAnalysis)),
        hasApproval: Boolean(storedApproval?.approvalId),
      });
      setStepIndex(Math.min(inferred.length, SSI_STEPS.length - 1));
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Unable to restore this SSI matter.");
      sessionStorage.removeItem("mailmypdf:appeal-ssi-denial:matter");
      setMatterId("");
    } finally {
      setBusy("");
    }
  ...
  useEffect(() => {
    ...
    const stored = fromUrl || sessionStorage.getItem("mailmypdf:appeal-ssi-denial:matter");
    if (!stored) return;
    setMatterId(stored);
    sessionStorage.setItem("mailmypdf:appeal-ssi-denial:matter", stored);
    void restore(stored);
  }, []);
  ...
  async function ensureMatter(): Promise<string> {
    ...
    const created = await createWorkflowCase(SSI_WORKFLOW_ID, SSI_VERTICAL_ID);
    setMatterId(created.id);
    sessionStorage.setItem("mailmypdf:appeal-ssi-denial:matter", created.id);
    return created.id;
  }
  ...
  async function uploadSource(files: File[]) {
    ...
    try {
      ...
      const uploaded = await uploadSecureWorkflowDocument({
        file,
        workflowId: SSI_WORKFLOW_ID,
        purpose: "ssi_denial_notice",
      });
      setDocuments(
  ...
  async function analyzeSource() {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "Unable to analyze the SSI decision.");
    } finally {
      setBusy("");
    }
  }
  ...
  async function checkout() {
    ...
    try {
      ...
      sessionStorage.setItem("mailmypdf:appeal-ssi-denial:order", result.orderId);
  ...
  function stepReady(step: SsiStepId): boolean {
  ...
  return (
    <StepShell
      breadcrumb={[
        ...
        { label: "Appeal SSI Denial", href: "/appeal-mail/workflows/appeal-ssi-denial" },
        { label: currentStep.label },
      ]}
      title="Appeal SSI Denial"
      subtitle="A source-grounded SSI reconsideration workflow using the real denial notice, claimant facts, medical or eligibility evidence, official SSA forms, exact packet review, mailing, and proof."
      lastSavedLabel={matterId ? "Matter saved" : undefined}
      steps={[...SSI_STEPS]}
      currentStepId={currentStep.id}
      completedStepIds={completedStepIds}
      onStepClick={(id) => {
        const target = SSI_STEPS.findIndex((step) => step.id === id);
        if (target >= 0 && (target <= stepIndex || completedStepIds.includes(id as SsiStepId))) setStepIndex(target);
      }}
      rail={rail}
    >
      ...
      {currentStep.id === "decision" && (
        <>
          <DocumentUpload
            title="1. Upload the SSI denial notice"
      ...
      {currentStep.id === "claimant" && (
        <SectionCard
          ...
          description="User-supplied facts are stored separately from AI-extracted notice facts. The questions adapt to the denial basis found in the notice."
        ...
        <>
          <SectionCard title="4. Supporting evidence" description="Choose the evidence category before upload. Only explicitly included clean documents enter the final packet.">
            <Field label="Evidence type">
              <select className="wf-input" value={evidenceKind} onChange={(event) => setEvidenceKind(event.target.value as SsiEvidenceKind)}>
                {SSI_EVIDENCE_KINDS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
              </select>
            </Field>
          </SectionCard>
          <DocumentUpload
            title="Upload evidence"
            items={evidence.filter((document) => !SSI_REQUIRED_FORMS.some((form) => form.kind === document.evidence_kind)).map((document) => ({
          ...
          />
          {evidence.filter((document) => !SSI_REQUIRED_FORMS.some((form) => form.kind === document.evidence_kind)).map((document) => (
            <CheckboxField
      ...
      <div className="wf-draft-actions">
        ...
        {currentStep.id !== "mail" && (
          <button className="wf-btn wf-btn--primary" type="button" disabled={!stepReady(currentStep.id)} onClick={() => setStepIndex((value) => Math.min(SSI_STEPS.length - 1, value + 1))}>Continue</button>
        )}
      </div>
    </StepShell>
  );
