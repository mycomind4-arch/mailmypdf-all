...
export type SsdiAppealStage = "reconsideration" | "hearing" | "appeals_council" | "unknown";
export type SsdiDecisionBasis = "medical" | "nonmedical" | "unknown";

export interface SsdiNoticeAnalysisOutput extends WorkflowMatterAnalysis["result"] {
  workflowDetails: {
    appealStage: SsdiAppealStage;
    decisionBasis: SsdiDecisionBasis;
    responseAddress: {
      line1: string;
      line2?: string | null;
      city: string;
...
export function isSsdiNoticeAnalysisOutput(value: unknown): value is SsdiNoticeAnalysisOutput {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const candidate = value as Record<string, unknown>;
...
export const SSDI_ANALYSIS_SYSTEM = [
  "You analyze Social Security disability decision notices for a document-grounded workflow.",
  ...
  "Never invent a date, deadline, diagnosis, impairment, address, appeal level, denial basis, evidence item, or reason.",
  ...
  "Classify decisionBasis only as medical, nonmedical, or unknown.",
  "Return JSON only.",
].join(" ");

export const SSDI_ANALYSIS_INSTRUCTION = `
Read the attached SSA denial/decision notice and return exactly this JSON shape:
{
...
The appeal level and decision basis must come from the notice itself. Do not infer them merely because the workflow is named SSDI Denial. If the notice does not clearly establish either classification, return unknown. Preserve the deadline wording from the notice in deadline; do not calculate a new deadline here.
`.trim();

export const SSDI_DRAFT_SYSTEM = [
  "You prepare a factual SSDI reconsideration cover/appeal letter from verified workflow state.",
  "Use only the source-notice analysis and claimant facts supplied in the task.",
  "Do not invent medical facts, symptoms, diagnoses, treatment, work limitations, dates, SSA rules, addresses, representatives, or evidence.",
...
function draftInstruction(analysis: WorkflowMatterAnalysis, caseInput: WorkflowRuntimeStoredInput): string {
  return [
    "Write the reconsideration letter now.",
...
export interface SsdiIntelligenceDependencies {
  ai: SecureAiGateway;
...
export function createSsdiIntelligenceGateway(deps: SsdiIntelligenceDependencies): WorkflowRuntimeIntelligenceGateway {
  return {
    async analyze({ actor, matter, source }) {
      ...
      const result = await deps.ai.execute({
        task: {
          id: `ssdi-analysis:${matter.matter.id}:${source.documentId}`,
          kind: "analyze",
          promptVersion: "appeal-ssdi-denial-analysis-v1",
          outputSchema: "ssdi-denial-analysis-v1",
          input: {
            system: SSDI_ANALYSIS_SYSTEM,
            instruction: SSDI_ANALYSIS_INSTRUCTION,
            outputMode: "json",
            media: [media],
        ...
        validateOutput: isSsdiNoticeAnalysisOutput,
      });
    ...
    async generateDraft({ actor, matter, analysis, caseInput }) {
      const result = await deps.ai.execute({
        task: {
          id: `ssdi-draft:${matter.matter.id}:analysis-${analysis.version}`,
          kind: "draft",
          promptVersion: "appeal-ssdi-denial-draft-v1",
          outputSchema: "ssdi-denial-draft-v1",
          input: {
            system: SSDI_DRAFT_SYSTEM,
      ...
      if (!validation.passed) {
        throw new Error(
          `Generated SSDI draft failed validation: ${validation.findings.filter((finding) => !finding.passed && (finding.severity === "error" || finding.severity === "block")).map((finding) => finding.detail).join("; ")}`,
        );
      }
