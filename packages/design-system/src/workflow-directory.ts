...
export interface PublicWorkflowDirectoryItem {
  ...
  meta?: string
  keywords?: string[]
  /** Use the workflow landing hero image here so the card thumbnail and hero stay identical. */
  imageSrc?: string
  imageAlt?: string
}
...
.mmp-directory-card__media{height:92px;position:relative;overflow:hidden;border-bottom:1px solid var(--mmp-border,#ded6c8);background:linear-gradient(135deg,color-mix(in srgb,var(--mmp-accent-soft,#e7f0f4) 80%,white),var(--mmp-paper-deep,#efe8dc))}.mmp-directory-card__media:after{content:'';position:absolute;right:-12px;top:-38px;width:118px;height:110px;border:1px solid color-mix(in srgb,var(--mmp-accent,#315f75) 18%,var(--mmp-border,#ded6c8));background:rgba(255,255,255,.52);transform:rotate(8deg);box-shadow:0 4px 14px rgba(16,40,61,.08)}.mmp-directory-card__media--image:after{display:none}.mmp-directory-card__media>img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;display:block}.mmp-directory-card__media--image:before{content:'';position:absolute;inset:0;z-index:1;background:linear-gradient(180deg,transparent 30%,rgba(8,24,37,.5) 100%)}.mmp-directory-card__media--image .mmp-directory-card__category{color:white;background:rgba(8,24,37,.66);border:1px solid rgba(255,255,255,.24)}
