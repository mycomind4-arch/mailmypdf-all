...
describe("WORKFLOW_EXECUTION_REGISTRY", () => {
  ...
  test("secured-transactions exposes only eligibility as executable", () => {
    const records = WORKFLOW_EXECUTION_REGISTRY.filter((r) => r.sectionId === "secured-transactions");
    assert.equal(records.length, 17);
    assert.equal(records.filter((r) => r.executionStatus === "executable").length, 1);
    assert.equal(isWorkflowExecutable("secured-transactions", "secured-transaction-eligibility"), true);
