...
import {
  ...
  type StudioVertical,
} from "@/studio/domain/studio-ecosystem";
import { studioProjects } from "@/studio/domain/studio-project";
import { findStepWorkflow } from "@/studio/domain/step-workflows";
import { readHiddenWorkflowIds, writeHiddenWorkflowIds } from "@/studio/lib/workflow-visibility";
import { scanProjectFiles, type StudioFileTreeNode } from "@/studio/lib/fns/scan-project-files";
import { scanWorkflowCatalog } from "@/studio/lib/fns/scan-workflow-catalog";
...
import { syncProjectToGithub } from "@/studio/lib/fns/sync-project-to-github";
import { publishProjectToCloudflare } from "@/studio/lib/fns/publish-project-to-cloudflare";
import {
  studioNodeKinds,
...
} from "@/studio/domain/studio-workflow";
...
async function studioFetch(input: RequestInfo | URL, init: RequestInit = {}) {
...
type StudioTreeNode = {
  path: string;
  label: string;
  kind: "core" | "verticals-folder" | "vertical" | "folder" | "workflows-folder" | "workflow";
  verticalId?: string;
  publicPath?: string;
  status?: string;
...
export function StudioPage() {
  ...
  const [, setExpandedTreePaths] = useState<Set<string>>(() => new Set(["private-office"]));
  ...
  async function loadCodeDirectory(path = "", showLoading = true) {
    ...
    try {
      const directory = await scanProjectFiles({ data: { projectId: activeProject.id, path: path || undefined }, headers: await authenticatedHeaders() });
  ...
  async function refreshCodeTree() {
    ...
    try {
      const directories = await Promise.all(
        paths.map(async (path) => ({ path, directory: await scanProjectFiles({ data: { projectId: activeProject.id, path: path || undefined }, headers: await authenticatedHeaders() }) })),
      );
  ...
  async function runAcceptanceTest(workflowId: string, scenario: string) {
    ...
    try {
      const response = await studioFetch("/api/studio/acceptance/test", {
        method: "POST",
    ...
    async function refresh() {
      try {
        const [sessionsResponse, runsResponse] = await Promise.all([
          studioFetch("/api/studio/chat/sessions"),
          studioFetch("/api/studio/agents/runs"),
        ]);
  ...
  useEffect(() => {
    ...
    void authenticatedHeaders().then((headers) => {
      if (cancelled) return;
      const token = headers.Authorization?.replace(/^Bearer\s+/, "");
      source = new EventSource(
        `/api/studio/agents/stream?runId=${encodeURIComponent(streamId)}&access_token=${encodeURIComponent(token ?? "")}`,
      );
  ...
  async function startChat() {
    ...
    try {
      ...
      const response = await studioFetch("/api/studio/chat/start", {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  async function dispatchAutonomousTeam() {
    ...
    try {
      const response = await studioFetch("/api/studio/agents/launch", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
  ...
  async function stopAutonomousTeam(runId: string) {
    try {
      await studioFetch("/api/studio/agents/stop", {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  async function sendChat() {
    ...
    try {
      const response = await studioFetch("/api/studio/chat/message", {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  async function runGate(gate: ChatGate) {
    ...
    setPendingGate(gate);
    try {
      const response = await studioFetch("/api/studio/chat/gate", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(
          gate === "reviewer"
  ...
  async function stopChat() {
    if (!activeSessionId) return;
    await studioFetch("/api/studio/chat/stop", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  async function syncWithGithub() {
    ...
    try {
      const check = await syncProjectToGithub({ data: { projectId: activeProject.id }, headers: await authenticatedHeaders() });
      ...
      const result = await syncProjectToGithub({ data: { projectId: activeProject.id, confirmed: true }, headers: await authenticatedHeaders() });
  ...
  async function streamPhase(phase: StudioPhase, reset = false) {
    ...
    const response = await studioFetch("/api/studio/run", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        id: phase.id,
