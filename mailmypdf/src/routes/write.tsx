...
import { validMailAddress, validMailEmail } from "@/lib/mail-review";
import { useMailReview } from "@/components/use-mail-review";
import { useMailOrder } from "@/components/use-mail-order";
import { useLetterDocument } from "@/components/use-letter-document";
import { useServerFn } from "@tanstack/react-start";
import { EmbeddedCheckoutProvider, EmbeddedCheckout } from "@stripe/react-stripe-js";
import { SiteHeader, SiteFooter } from "@/components/site-chrome";
import { PaymentTestModeBanner } from "@/components/payment-test-mode-banner";
import { createCheckoutForOrder, previewLetterPricing } from "@/lib/orders.functions";
...
export const Route = createFileRoute("/write")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Type your letter online. We print and mail it for you — no printer needed.",
      },
    ],
  }),
  component: WritePage,
...
function WritePage() {
  ...
  const prepared = useLetterDocument(step >= 2, letterText, sender, recipient);
  const actualPages = prepared.document?.pages ?? pageCount;
  const reviewInputs = JSON.stringify([
    letterText,
    sender,
    recipient,
    email,
  ...
  const priceCents =
    quote?.basis === reviewInputs
      ? quote.cents
      : calculateTotalPrice({ pageCount: actualPages, color, mailClass });
  ...
  const estimatePricing = async () => {
    ...
    try {
      ...
      /* ignore — will re-estimate on submit */
    }
  };

  const runAI = async (
    action: "generate" | "improve" | "formal" | "friendly" | "firm" | "shorten" | "expand",
    prompt?: string,
  ) => {
    setAiLoading(true);
    setAiError(null);
    try {
      ...
      if (result.error) {
        ...
        try {
          ...
          /* pricing preview is best-effort */
        }
      }
    } catch (e) {
  ...
  const startCheckout = async () => {
    if (!prepared.document || !canGoTo(3) || checkoutLock.current) return;
    checkoutLock.current = true;
    setSubmitting(true);
    setError(null);
    try {
      const pdf = prepared.document.file;
      ...
      const created = await prepareOrder({
        ...
        file: { name: pdf.name, sizeBytes: pdf.size, dataBase64: btoa(binary) },
        color,
        mailClass,
      });
      if (!created) {
  ...
  const fetchClientSecret = useCallback(async (): Promise<string> => {
    ...
    const result = await createCheckoutFn({
      data: { orderId: checkout.orderId, token: checkout.token, approvedPriceCents: priceCents },
    });
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-3xl px-6 py-12">
        ...
        <fieldset disabled={submitting} className="mt-10 min-w-0">
          ...
          {step === 2 && !prepared.document && (
            <div role={prepared.error ? "alert" : "status"} className="envelope-card p-6">
              <p>{prepared.error ?? "Preparing the exact PDF and page count for your review…"}</p>
              <button
                type="button"
                onClick={() => setStep(1)}
          ...
          {step === 2 && prepared.document && (
            <>
              ...
              <PdfDocumentReview file={prepared.document.file} />
              <ReviewStep
                letterText={letterText}
                sender={sender}
                recipient={recipient}
    ...
    <section>
      ...
      <div className="flex items-center justify-between">
        <p className="mt-2 text-muted-foreground">
          Type your letter below. We'll print it on professional paper and mail it for you. Need
          inspiration?{" "}
          <a href="/templates" className="text-cobalt underline">
            Browse templates
          </a>
          .
        </p>
      ...
      {props.showAI && (
        <div className="mt-6 rounded-lg border border-cobalt/25 bg-stamp/5 p-5">
          <div className="flex items-center gap-2">
            ...
            {props.aiLoading && (
              <span className="text-xs text-muted-foreground animate-pulse">Writing...</span>
            )}
          </div>
          ...
          <div className="mt-3">
            <label className="text-xs font-medium text-muted-foreground">
              Describe your letter and let AI write it:
            </label>
          ...
          <div className="mt-3">
            <label className="text-xs font-medium text-muted-foreground">
              Or improve your existing letter:
            </label>
      ...
      <div className="mt-4 flex items-center justify-between text-sm">
        <span className="text-muted-foreground">
          {props.letterText.trim()
            ? `~${props.pageCount} page${props.pageCount === 1 ? "" : "s"}`
            : "Start typing to estimate pages"}
        </span>
    ...
    <section>
      ...
      <div className="mt-8 flex gap-3">
        <button
          ...
          className="rounded-full border border-rule px-6 py-2.5 text-sm font-medium transition-colors hover:border-ink"
        >
          ← Back
        </button>
        <button
          onClick={props.onNext}
          className="flex-1 rounded-full bg-primary py-2.5 text-center text-sm font-medium text-primary-foreground transition-transform hover:-translate-y-0.5"
        >
          Review your letter →
        </button>
      </div>
    ...
    <div>
      ...
      <div className="mt-3 space-y-3">
        <input
          ...
          className="w-full rounded-lg border border-rule bg-card px-3 py-2 text-sm focus:border-stamp focus:outline-none"
        />
        <input
          value={address.line1}
        ...
        <div className="flex gap-2">
          ...
          <input
            ...
            placeholder="ST"
            maxLength={2}
            className="w-16 rounded-lg border border-rule bg-card px-3 py-2 text-sm focus:border-stamp focus:outline-none"
          />
        </div>
        <input
  ...
  return (
    <section>
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Address fields are checked for format, not confirmed postal deliverability. Review the
        formatted PDF above; unsupported characters may render differently from your original text.
      </p>

      <div className="mt-8 space-y-6">
        <div className="rounded-lg border border-rule bg-card p-6">
          <h3 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
            Your original text — review the formatted PDF above
          </h3>
          <pre className="mt-3 max-h-64 overflow-auto whitespace-pre-wrap font-serif text-sm leading-relaxed text-foreground">
            {props.letterText}
          </pre>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-lg border border-rule bg-card p-4">
            ...
            <p className="mt-2 text-sm">
              ...
              {props.sender.city}, {props.sender.state} {props.sender.postalCode}
            </p>
          </div>
          <div className="rounded-lg border border-rule bg-card p-4">
            <h3 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
            ...
            <p className="mt-2 text-sm">
              ...
              <br />
              {props.recipient.line1}
              <br />
              {props.recipient.line2 ? (
                <>
                  {props.recipient.line2}
                  <br />
                </>
              ) : null}
              {props.recipient.city}, {props.recipient.state} {props.recipient.postalCode}
            </p>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-rule bg-card p-6">
          ...
          <div className="mt-1 flex justify-between text-sm">
            <span>Color</span>
            <span>{props.color ? "Yes" : "No (B&W)"}</span>
          </div>
          <div className="mt-1 flex justify-between text-sm">
            <span>Delivery</span>
            <span>{MAIL_CLASS_LABELS[props.mailClass]}</span>
          </div>
          <div className="mt-3 border-t border-rule pt-3 flex justify-between text-base font-medium">
            <span>Total</span>
            <span>{formatUSD(props.priceCents)}</span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              type="checkbox"
              checked={props.agreedReviewed}
              onChange={(e) => props.setAgreedReviewed(e.target.checked)}
              className="mt-0.5 h-4 w-4 rounded border-rule accent-stamp"
            />
            <span className="text-sm text-muted-foreground">
              I've reviewed every page of the formatted PDF, both addresses, and the total price.
            </span>
          </label>
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              type="checkbox"
              checked={props.agreedContent}
              onChange={(e) => props.setAgreedContent(e.target.checked)}
              className="mt-0.5 h-4 w-4 rounded border-rule accent-stamp"
            />
            <span className="text-sm text-muted-foreground">
              I understand MailMyPDF prints and mails exactly what I wrote. I'm responsible for the
              content.
            </span>
          </label>
        </div>
      ...
      <div className="mt-8 flex gap-3">
        ...
        <button
          onClick={props.onNext}
          disabled={!(props.agreedReviewed && props.agreedContent) || props.submitting}
          className="flex-1 rounded-full bg-primary py-2.5 text-center text-sm font-medium text-primary-foreground transition-transform hover:-translate-y-0.5 disabled:opacity-40 disabled:hover:translate-y-0"
        >
          {props.submitting
            ? "Preparing checkout..."
            : `Continue to checkout · ${formatUSD(props.priceCents)} →`}
        </button>
      </div>
    </section>
  );
...
function PayStep({
  ...
  return (
    <section>
      ...
      <p className="mt-2 text-muted-foreground">
        Secure checkout via Stripe — {formatUSD(priceCents)}
      </p>
      ...
      <button
        onClick={onBack}
        className="mt-6 rounded-full border border-rule px-6 py-2.5 text-sm font-medium transition-colors hover:border-ink"
      >
        ← Back
      </button>
    </section>
