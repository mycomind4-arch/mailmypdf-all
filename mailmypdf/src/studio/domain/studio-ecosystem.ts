...
export type StudioCatalogWorkflow = {
  ...
  /**
   * New root-level architecture signal (see context/CURRENT_WORK.md), filled
   * in by `scanWorkflowCatalog` alongside `onDisk`. A workflow can have a
   * real config.ts under `<root>/workflows/<id>/` yet still be completely
   * unreachable in the live app if nothing mounts it — see
   * `context/FACTORY_STATUS.md`'s 2026-09-20 entry, which is exactly the gap
   * CP14 exposed. `configured` alone is not "done"; `mounted` is required
   * for the page to actually serve.
   */
  newArchitecture?: { configured: boolean; mounted: boolean };
};
...
const registeredWorkflowRows: RegisteredWorkflowRow[] = [
  ["notice-respond", "cp2000-response", "Respond to an IRS CP2000 Notice", "/notice-respond/workflows/cp2000-response", "functional"],
  ["notice-respond", "cp14-response", "Respond to an IRS CP14 Notice", "/notice-respond/workflows/cp14-response", "authority"],
  ...
  ["notice-respond", "cp504-response", "Respond to an IRS CP504 Notice", "/notice-respond/workflows/cp504-response", "functional"],
